			...
			<br>
				...
				<div class="tab-content">
					<div role="tabpanel" class="tab-pane" id="panel-inicio">
							...
							<br> <a href="?modulo=pipa&controller=pipa&action=pesquisaPmda&a=adm" class="btn btn-primary" title="Voltar Menu ">Voltar</a>
							<a onclick="javascript:mensagem();" class="btn btn-default" 
								title="Visualizar Histórico de Mensagens do sistema">Histórico de Mensagem</a>
							<p class="pull-right">
								<?php if($pmda->verificaCriarPmda($id_municipio) == 0){
									print "<button type=\"button\" class=\"btn btn-primary\" title=\"Criar novo PMDA\" id=\"addPmda\">Novo PMDA</button>";
								}else {
									print "<br><span class=\"alert alert-danger\">Não é possível criar um novo PMDA pois existe PMDA a ser Editado Verifique a lista Abaixo \"Histórico dos PMDA\" !</span>&nbsp;&nbsp;";
									//print "<button class=\"btn btn-danger\" title=\"Não é possivel criar novo PMDA, pois já exite PMDA em Edição \">Novo PMDA</button>";
								}
								?>
								</p>
								<br><br>

							<div style="v-align: center">
...
$(document).ready(function(){
	...
	$("#btnCadastrarComunidade").click(function(){
		NovaJanela('?modulo=pipa&controller=pipa&action=precadcom','600','500');
	});
	...
	/****************  reload ponto captaçao ************/
		$("#tab_comunidade").click(function(){
			...
			var id_pmda = getUrlVars()['param'];
			//alert(id_pmda);
			$("#tblComunidadePmda").load("/mod_pipa/View/pmda/comunidade.php?param="+id_pmda);

			$.ajax({
			       url : 'mod_pipa/View/pmda/selPonto.php',
			       type : 'POST',
			       data : dados,
			       success : function(response) {
			    ...
			    success : function(response) {
			      	if(response == "sucesso"){
						...
						if(result) {
							...
							$.ajax({
							       url : '/mod_pipa/View/pmda/municipio.php',
							       type : 'POST',
							       data : dados,
							       //dataType : 'json',
							       success : function(response) {
			    ...
			    //inicio
		      	if(response == "sucesso"){
					...
					if(
						...
						$.ajax({
						       url : '/mod_pipa/View/pmda/acoes.php',
						       type : 'POST',
						       data : dados,
						       success : function(response) {
						    	   alert('Dados gravados com Sucesso !');
			      	...
			      	if(response == "sucesso"){
						...
						if(($('#txtAnexoDesc').val() == "") && ($('#fileAnexo').val() == "")){
							...
							if(extensao.toLowerCase() == 'pdf' ||
								...
								$.ajax({
								       url : '/mod_pipa/View/pmda/anexo.php',
								       type : 'POST',
								       data : formData,
		      	...
		      	if(response == "sucesso"){
			    	...
			    	var dados = {"envia" :"novo",
			            ...
			            $.ajax({
			                type: 'POST',
			                //dataType: 'json',
			                url: '/mod_pipa/View/pmda/novo.php?v=<?=md5(VERSAO)?>',
			                data: dados,
			                success: function(response) {
				                console.log(dados);
								location.reload();
		    ...
		    success : function(response) {
		      	if(response == "sucesso"){
					...
					if(
		                ...
		                $.ajax({
		                    type: 'POST',
		                    url: '/mod_pipa/View/pmda/ponto.php?v=<?=md5(VERSAO)?>',
		                    data: dados,
		                    success: function(response) {
		    ...
		    success : function(response) {
		      	if(response == "sucesso"){
					...
					if(
		                ...
		                $.ajax({
		                    type: 'POST',
		                    url: '/mod_pipa/View/pmda/membroEquipe.php?v=<?=md5(VERSAO)?>',
		                    data: dados,
		                    //dataType: 'json',
		                    success: function(response) {
		                        alert("Registro adicionado com sucesso !");
					...
					if (
						...
						alert("Existem campos em branco");
					    	...
					    	if(result == true){
					                ...
					                $.ajax({
					                    type: 'POST',
					                    url: '/mod_pipa/View/pmda/comunidade.php?v=<?=md5(VERSAO)?>',
					                    data: dados,
					                    //dataType : 'json',
					                    success: function(response) {
						                    alert("Registro alterado com sucesso !");
		      	...
		      	if(response == "sucesso"){
					$("#txtIdPmda").val(<?=isset($id_pmda)? $id_pmda: "";?>)
			            ...
			            if(
		    	            ...
		    	            $.ajax({
		    	                type: 'POST',
		    	                url: '/mod_pipa/View/pmda/representante.php?v=<?=md5(VERSAO)?>',
		    	                data: dados,
		    	                success: function(response) {
		    ...
		    success : function(response) {
		      	if(response == "sucesso"){
			    	...
			    	if(confirm1 == true){
			            ...
			            $.ajax({
			                type: 'POST',
			                url: '/mod_pipa/View/pmda/anexo.php?v=<?=md5(VERSAO)?>',
			                data: dados,
			                success: function(response) {
	    ...
	    success : function(response) {
	      	if(response == "sucesso"){
				...
				if(result){
					...
					$.ajax({
				        type: 'POST',
				        url: '/mod_pipa/View/pmda/homologacao.php?v=<?=md5(VERSAO)?>',
				        data: dados,
				        success: function(response) {
					        //console.log(response);
				        	location.reload();
	...
	success : function(response) {
		  if(response == "sucesso"){
				...
				$.ajax({
					...
					data: dados,
					success: function(response) {
							window.location.href = '?token=<?=hash("sha256", md5(VERSAO).date('dmY'));?>&ac=&modulo=pipa&controller=pipa&action=pmda&param='+id_pmda+'&p='+protocolo+'&mun='+id_municipio;
					},
					error: function(response){
						console.log(JSON.stringify(response));
					}
	...
	success : function(response) {
		  if(response == "sucesso"){
				...
				$.ajax({
					...
					data: dados,
					success: function(response) {
							window.location.href = '?modulo=pipa&controller=pipa&action=mensagem';
					},
					error: function(response){
						console.log(JSON.stringify(response));
					}
	...
	success : function(response) {
		  if(response == "sucesso"){
				...
				$.ajax({
					...
					data: dados,
					success: function(response) {
							window.location.href = '?modulo=pipa&controller=pipa&action=printPmda&param='+id_pmda+'&mun='+id_municipio;
					},
					error: function(response){
						console.log(JSON.stringify(response));
					}
...
function termo_compromisso(){
	window.location.href = 'index.php?token=<?=hash('sha256', md5(VERSAO).date('dmY'));?>&ac=&modulo=pipa&controller=pipa&action=termo&param=<?=$id_pmda;?>&mun=<?=$id_municipio;?>';
...
function declaracaoiss(){
	window.location.href = 'index.php?token=<?=hash('sha256', md5(VERSAO).date('dmY'));?>&ac=&modulo=pipa&controller=pipa&action=declaracaoiss&param=<?=$id_pmda;?>&mun=<?=$id_municipio;?>';
...
function lerMensagemRecebida(id_pmda, status, opcao){
	...
	window.location.href = 'index.php?token=<?=hash('sha256', md5(VERSAO).date('dmY'));?>&ac=&modulo=pipa&controller=pipa&action=mensagem&id_pmda='+id_pmda+'&status='+status+'&opcao='+opcao;
