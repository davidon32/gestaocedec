...
class TransferenciaMaterial {
    ...
    /* get dados transferencia */

    public static function getSituacaoTransferencia($id_transferencia) {
        ...
        $sql = "Select situacao "
                . "from aju_transferencia where id_transferencia = ".$id_transferencia;

        $result = $con->query($sql);

        while ($linha = $result->fetch(PDO::FETCH_ASSOC)) {
            $dados = $linha['situacao'];
        }
        return $dados;
    }
