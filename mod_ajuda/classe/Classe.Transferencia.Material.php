	...
	function receberMaterial($id_deposito_destino, $_nivel){
		...
		while($linha = $result->fetch(PDO::FETCH_ASSOC)){
			...
			print "<tr>
						...
						<td><a class=\"btn btn-primary\" href=\"index.php?token=".hash('sha256', md5(VERSAO))."&ac=itn&modulo=ajuda&secao=recebimento&acao=receber&id=".$linha['id_transferencia']."\"><i class=\"\">Receber</i></a></td>
					</tr>";
		}
		...
		try{
			...
			$nlinha = $result->rowCount();
				...
				if($nlinha > 0){
	                ...
	                return "<a href=\"javascript:NovaJanela('index.php?token=".hash('sha256', md5(VERSAO))."&ac=itn&modulo=ajuda&controller=conEstoque&action=lembrete_transito&id=".$dados['id_transferencia']."', 700, 300)\"><img src='mod_ajuda/imagem/transito.png' title='Existe Material em Transito '></a>";
				}
		}catch (Exception $e){
