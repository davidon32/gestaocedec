...
$tramit = isset($_GET['tramit']) ? $_GET['tramit'] : null;
...
<div class="col col-md-12">
    <a class="btn btn-success" href="?token=<?= hash('sha256', md5(VERSAO) . date('dmY')) ?>&modulo=ajuda&controller=h_pedido_index&action=menu">Voltar</a>
</div>
    ...
    <div class="row">
        <div class="col-md-12">
            ...
            <hr>
                <div class="row">
                    <div class="col">
                        <?=($_SERVER['REQUEST_METHOD'] == "GET")  ? "<p><span class='bold'>50 Últimos Registro de Pedidos</span></p>" : ""?>
                    </div>
                </div>
        </div>
        <div class="row">
            ...
            <?php $municipios = Municipio::dadosSelectMunicipio()?>
            <div class="col-md-6 p-2">
                <form action="<?=FuncaoBase::geraLink('ajuda','h_pedido_index', 'index')?>" method="post" name="frmPedido" id="frmPedido">
                    <label>Busca Processos : </label><br>
                    <select name="select_municipio" id="select_municipio" class="form form-control js-example-basic-multiple">
                        <option>Selecione o municipio</option>
                        <?php foreach ($municipios as $municipio) {
                            print "<option value='".$municipio['nome']."'>".$municipio['nome']."</option>";
                        }
                        ?>
                    </select>
                    <br><br>
            ...
            <div class="col-md-12 table-responsive">
                <table id="pedidos" class="table table-bordered table-sm table-condensed table-responsive dataTable">
                    ...
                    $municipio = isset($_POST['select_municipio']) ? $_POST['select_municipio'] : null;

                    //var_dump($municipio, $id_redec);

                    if ($secao == 'REDEC') {
                        $listaPedido1 = $pedido_h->listaPedidosTodos($municipio, $id_redec, null);
                    } elseif(!is_null($status)) {
                        $listaPedido1 = $pedido_h->listaPedidosTodos($municipio, null, $status);
                    }elseif(!is_null($tramit)) {
                        $listaPedido1 = $pedido_h->listaPedidosTodos($municipio, null, null, $tramit);
                    }else {
                        $listaPedido1 = $pedido_h->listaPedidosTodos($municipio, null, null);
                    }
                    ...
                    $total_reg = 0;
                        ...
                        foreach ($listaPedido1 as $key => $pedid) {
                            # get permissao
                            ...
                            if (($pedid['tramit'] == 'analise_drd' && $permissao[0]['analista_drd'] == '1') ||
                                ...
                                ($pedid['tramit'] == 'reprovado')||
                                ($pedid['tramit'] == 'finalizado')
                        ...
                        'columns': [{
                            ...
                            },
                            {
                                ...
                                render: function(data, type, row) {
                                    ...
                                    var links_opcoes = '<a href=\'' + geraLink('ajuda', 'h_pedido_pedid', 'view', '<?= VERSAO ?>', {
                                        id: data.id,
                                        voltar: 'idx_recente'
                                    }) + '\' title=\'Visualiação e Impressão do Pedido\'><img width=\'25px\' src=\'/core/imagem/view1.png\'></a>|';
                                    ...
                                    if ((data.status > 0 && data.status <= 5 || users[1].id_usuario == 1 || users[1].secao == 'CHEFIA' || users[1].secao == 'DLOG') && (users[1].secao != 'REDEC')) {
                                        links_opcoes += '<a href=\'' + geraLink('ajuda', 'h_pedido_pedid', 'edit', '<?= VERSAO ?>', {
                                            id: data.id,
                                            voltar: 'idx_recente',
                                            aba: 'dadosgerais'
                                        }) + '\' title=\'Editar Pedido\'><img src=\'/core/imagem/editar.png\'></a>';
                                    ...
                                    //     links_opcoes += '<a href=\'' + geraLink('ajuda', 'h_pedido_prest', 'index', '<?= VERSAO ?>', {
                                    //         id: data.id
                                    //     }) + '\' title=\'Presatação de contas\'><img width=\'25\' src=\'/core/imagem/relatorio.png\'></a>';
                                    //     links_opcoes += '&nbsp;&nbsp;<a href=\'' + geraLink('ajuda', 'h_pedido_prest', 'index', '<?= VERSAO ?>', {
                                    //         id: data.id
                                    //     }) + '\' style=\'color:#ffffff; font-size:14pt\' title=\'Percentual de Conclusão da Prestação de Contas do Pedido\'><i style=\'font-size:10pt;\'>' + data.percent + '%</i></a> ';
                                    // }
