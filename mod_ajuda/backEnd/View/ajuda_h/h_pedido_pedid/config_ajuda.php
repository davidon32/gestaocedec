...
<div class="col-md-12">
    ...
    <div class="row">
        <div class="col-md-6">
            ...
            <label>Prazo Prestação de Contas ( Padrão X Dias )</label><br>
        ...
        <div class="col-md-6">           
            <label>Materiais que podem ser solicitados</label>

            
            <div class="input-group">
                <input type="search" class="form col-md-12" name='buscaMat' id='buscaMat' data-opcao='1' data-id_unidade='0'>
                <span class="input-group-btn">
                    
                    <button name='btnAutorizar' id='btnAutorizar'  class="btn btn-primary" type="button"><span class="glyphicon glyphicon-search" aria-hidden="true" title="Liberar Material para Pedidos">
                        </span> Autorizar !</button>
                </span>
            </div>
            <br>
            <table class="table table-bordered">
                <tr><th colspan="5" class="text-center">Materiais Permitidos para Pedidos AH</th></tr>
                <tr>
                ...
                <?php
                    ...
                    $listMatDisp = $h_pedido_pedid->MaterialPedido(1);
                    
                    foreach ($listMatDisp as $key => $value) {
...
<script>
    ...
    $(document).ready(function () {
        ...
        var itens = {
            data:
<?php print json_encode($listMateriais); ?>, // array com os dados
            getValue: "singular", /* alterar com nome do item BD */
            list: {
                match: {
                    enabled: true
                },
        ...
        /*********** autocomplete ***********/
        $("#buscaMat").easyAutocomplete(itens);


        $("#btnAutorizar").click(function(){
            var id = $('#buscaMat').data('id_unidade');
            var opcao = $('#buscaMat').data('opcao');
            permissaoMaterial(id, opcao);
        });

    });
    
    /* adicionar permissao material */
    function permissaoMaterial(id, func){
    
        var formData = new FormData();
            formData.append('opcao', 'permis_material_pedido');
            formData.append('func', func);
            formData.append('id_unidade', id);
