...
<div class="col-md-12">
    ...
    <table class="table ">
        ...
        <tbody>
            ...
            foreach ($dados as $value) {
                ...
                $tramitacao = $pedido_h->listTramitacao($value['id']);
                
                $title_tramitacao = '';
                $title_tramitacao_fonte = '';
                $data = '<tr>
                            <th>#</th>
                            <th>Operador</th>
                            <th>Texto</th>
                ...
                if( (count($tramitacao) > 0) && ($value['status'] == 0 ) ){
                    
                    $title_tramitacao = "Pedido Devolvido para Correções e Reenvio !" ;
                    $title_tramitacao_fonte = " style='color:red;font-weight:bolder' " ;

                    foreach ($tramitacao as $key1 => $linha) {
                        $data .= "<tr><td>" . ($key1 + 1) . "</td><td>" . Usuario::getNomeId($linha['id_usuario']) . "</td>
                                            <td>" . $linha['obs'] . "</td>
                                            <td>" . DataMysql::dataCompletaVisual($linha['dt_tramita']) . "</td>
                                        </tr>";
                    }
                }
            ...
            <td title='" . $value['id'] . "' ".$title_tramitacao_fonte.">" . $value['numero'] . "-" . substr($value['data_entrada_sistema'], 0, 4) . "</td>
            <td title='".$title_tramitacao."'".$title_tramitacao_fonte.">" . DataMysql::dataCompletaVisual($value['data_entrada_sistema']) . "</td>
            <td title='".$title_tramitacao."'".$title_tramitacao_fonte.">" . Decreto::getNomeCobrade($value['id_cobrade']) . "</td>
            <td title='".$title_tramitacao."'".$title_tramitacao_fonte.">" . (($value['despachante_analista'] == "") ? "-   " : $value['despachante_analista']) . "</td>
            <td title='".$title_tramitacao."'".$title_tramitacao_fonte.">";
                if( ($value['status'] == 0) && (count($tramitacao) > 0) ) {
                    print 'Devolvido';
                }else {
                 print $pedido_h->enumStatus($value['status']);
                }
            print "</td>
            <td title='".$title_tramitacao."'".$title_tramitacao_fonte.">" . DataMysql::dataCompletaVisual($value['data_hora_envio']) . "</td>
            <td title='".$title_tramitacao."'".$title_tramitacao_fonte.">";
                ...
                if (($value['status'] == 0) && (count($tramitacao) > 0)) {
                    print "&nbsp;<button type='button' name='btn-mensagem' class='btn btn-link' data-toggle='modal' data-target='#mensagemModal1' data-id='" . $data . "'><img src='/core/imagem/tramit-msg.png' width='25' title='Mensagem do Analista sobre a Devolução do pedido para Edição !'></button>";
                }
    ...
    <!-- Modal -->
    <div class="modal fade bs-example-modal-lg" id="mensagemModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel">Mensagem(s) Sobre devolução do Pedido para Edição</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <div class="modal-body">
                    <table class="table table-sm lista-mensagem" id="list-data">
                    </table>
                </div>
...
<script>
    ...
    $(document).ready(function () {
        ...
        $("button[name='btn-mensagem']").click(function () {
            var dados = $(this).data('id');
            console.log(dados);
            $('#mensagemModal').on('show.bs.modal', function (e) {
                $('#mensagemModal').children().children().children().children('table').empty();
                $('#mensagemModal').children().children().children().children('table').append(dados);
            }).modal('show');
