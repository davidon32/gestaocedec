...
if (MANUTENCAO && $_SERVER['SERVER_NAME'] != 'desenvolvimento.gestaocedec') {
    ...
    $token = isset($_GET['token1']) ? $_GET['token1'] : false;
    $tokenEx = isset($_GET['tokenEx']) ? $_GET['tokenEx'] : false;
    ...
    /* usuario CEDEC */
    if ($token) {
        $logarCedec = Login::logarToken($token);
        
        if($logarCedec) {
            #redireciona
            header('Location: index.php?modulo=index&controller=index&action=index1');
        }
    
    /* usuario externo */
    }elseif($tokenEx) {

        $logarEx = LoginExterno::logarTokenEx($tokenEx);

        if($logarEx) {
            # redirect
            header('Location: index.php?modulo=index&controller=index&action=index1e');
        }
    }else {
        //die();
        //$logarEx = LoginExterno::logarTokenEx($token);


        //var_dump($token, 'admin: '.$logarCedec );

        //var_dump($token);
        //die();

        //     if ($logarCedec == 'indexAdm') {

        //         var_dump($modulo);
        //         var_dump($controller);
        //         var_dump($action = 'index1');
        //         var_dump($action);

        //         //die();
        //         include_once "mod_" . $modulo . "/Controller/" . $controller . ".php";

        //         //print $_SERVER['HTTP_HOST']."/index.php?".FuncaoBase::geraLink('index', 'index', 'index1');

        //         //header('Location:index.php?ac=itn&modulo=index&controller=index&action=index1e');

        //         //header("Location: http://".$_SERVER['HTTP_HOST']."/index.php".FuncaoBase::geraLink('index', 'index', 'index1'));
        //         //die();
        //         //exit();
        //     //} elseif ($logarEx == 'indexEx') {
        //    // }else {
        //    //     print "<scritp>alert('Usuário ou Senha Erradas !')</script>";
        //    // }
            ...
            if ((isset($caminho[2]) && ($caminho[2] === 'mapa')) && ((isset($caminho[3]) && $caminho[3] === 'site'))) { # mapas
                ...
                if ($controller == 'novoUserController') {
                    ...
                    if (isset($_COOKIE['SEGURANCA'])) {
                        ...
                        //print "<script>window.location.href = 'http://sdc.net:8081/index.php';</script>";
                ...
                //var_dump(file_exists("mod_" . $modulo . "/Controller/" . $controller . ".php"));
                //die("erro");
            }
        }
    }
